...
from nstt_course_planner.bathrooms import BATHROOM_STOPS, BathroomCategory, BathroomLayerBuilder, RunnerRouteKml
...
def test_official_beach_stops_use_public_agency_sources() -> None:
    official_hosts = ("santamonica.gov", "longbeach.gov", "parks.ca.gov", "carlsbadca.gov", "encinitasca.gov", "sandiego.gov")
...
def test_builder_preserves_source_and_writes_ranked_kml(tmp_path: Path) -> None:
    ...
    BathroomLayerBuilder().build(source, output)
