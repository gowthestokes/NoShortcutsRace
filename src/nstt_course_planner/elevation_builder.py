...
class ElevationLayerBuilder:
    ...
    def add_arguments(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            help="Directory for NSTT_2026_elevation.kml.",
        )
        parser.add_argument(
            "--elevation-cache",
            type=Path,
            default=DEFAULT_GOOGLE_ELEVATION_CACHE,
            help="Local cache of sampled terrain elevations.",
        )
        parser.add_argument(
            "--elevation-usage",
            type=Path,
            default=DEFAULT_GOOGLE_ELEVATION_USAGE,
            help="Local conservative Google Elevation sample counter.",
        )
        parser.add_argument(
            "--sample-spacing-meters",
            type=float,
            default=DEFAULT_ELEVATION_SAMPLE_METERS,
            help="Terrain sample spacing; 50 m is the default.",
        )
        parser.add_argument(
            "--smoothing-meters",
            type=float,
            default=DEFAULT_ELEVATION_SMOOTHING_METERS,
            help="Elevation smoothing window, from 150 to 250 m.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def _section_runs(self):
        ...
        if not runs:
            raise RuntimeError(
                f"Source KML has no editable runner segments: {self.config.source_kml}",
            )
        return runs
    ...
    def _sample_coordinates(self, section_runs) -> list[tuple[float, float]]:
        ...
        for sections in section_runs:
            ...
            coordinates.extend(
                sample.coordinate
                for sample in RouteGeometrySampler.sample(
                    run_coordinates,
    ...
    def build(self) -> None:
        if not self.config.source_kml.exists():
            raise RuntimeError(
                f"Finalized runner KML was not found: {self.config.source_kml}",
            )
