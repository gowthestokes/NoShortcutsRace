...
class RunnerRouter:
    ...
    def highway_1_via_points_from(start: Point) -> tuple[tuple[float, float], ...]:
        return tuple(
            point for point in HIGHWAY_1_VIA_POINTS if point[1] > start.longitude
        )
    ...
    def pedestrian_route_payload(chunk: list[Point]) -> dict[str, object]:
        return {
            "locations": [
                {"lat": point.latitude, "lon": point.longitude} for point in chunk
            ],
            "costing": "pedestrian",
    ...
    @classmethod
    def google_walking_route_payload(
        cls,
        start: Point,
        end: Point,
    ) -> dict[str, object]:
    ...
    @staticmethod
    def require_no_ferry_instruction(
        ...
        if any("ferry" in instruction.casefold() for instruction in instructions):
            raise UnsafePedestrianRouteError(
                f"Walking route from {start.label} to {end.label} includes a ferry instruction; confirm the runner-accessible land route before exporting a map.",
            )

    @staticmethod
    def require_land_based_pedestrian_trip(
        ...
        if isinstance(summary, dict) and summary.get("has_ferry"):
            raise UnsafePedestrianRouteError(
                f"Pedestrian route from {start.label} to {end.label} uses a ferry; confirm the runner-accessible land route before exporting a map.",
            )

    @staticmethod
    def google_route_distance_meters(
    ...
    def google_route(
        ...
        *,
        ...
        try:
            ...
            GoogleUsageTracker.set_status(
                self.usage,
                self.usage_path,
                f"failed HTTP {error.code}",
            )
            raise RuntimeError(
                f"Google Routes request failed with HTTP {error.code}; check the key and Routes API setup.",
            ) from error
            ...
            raise RuntimeError(
                "Google Routes request failed due to a network error.",
            ) from error
        ...
        if not routes:
            ...
            raise RuntimeError(
                f"Google Routes returned no {travel_mode.casefold()} route from {start.label} to {end.label}.",
            )
        route = routes[0]
        ...
        if not isinstance(polyline, str):
            GoogleUsageTracker.set_status(
                self.usage,
                self.usage_path,
                "failed: missing polyline",
            )
            raise TypeError("Google Routes returned a route without geometry.")
        instructions = [
        ...
        for index in range(len(points) - 1):
            ...
            if self.is_i5_support_car_transfer_pair(start, end):
                ...
                if segments[-1]:
                    segments.append([])
                continue
            if self.is_highway_1_pair(start, end) or manual_highway:
                mode, variant = (
                    "DRIVE",
                    "manual-highway-1-pch-v2" if manual_highway else "highway-1-pch-v2",
                )
                vias = (
                    self.highway_1_via_points_from(start)
                    if manual_highway
                    else HIGHWAY_1_VIA_POINTS
                )
            elif self.is_strict_road_pair(start, end):
                mode, variant, vias = "DRIVE", "organizer-turn-v1", ()
            else:
                mode, variant, vias = "WALK", "", ()
            cache_key = self.route_cache_key(
                start,
                end,
                travel_mode=mode,
                variant=variant,
            ...
            if cached := self.routing_cache.get(cache_key):
                geometry = [
                    ...
                    for latitude, longitude in cached["geometry"]
                ]
                distance_meters = float(cached["distance_meters"])
            else:
                geometry, distance_meters = self.google_route(
                    start,
                    end,
                    travel_mode=mode,
                    via_points=vias,
