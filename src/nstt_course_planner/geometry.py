...
class RouteGeometry:
    ...
    def distance_meters(cls, segments: list[list[tuple[float, float]]]) -> float:
        return sum(
            cls.haversine_meters(start, end)
            for segment in segments
            for start, end in pairwise(segment)
        )
    ...
    def interpolate(
        ...
        ) * fraction, start_longitude + (end_longitude - start_longitude) * fraction
...
class RouteSegmenter:
    ...
    def sections(
        ...
        *,
        ...
        interval_meters: float = MILE_METERS,
        ...
        for segment_index, segment in enumerate(segments):
            ...
            for start, end in pairwise(segment):
                edge_meters = (
                    RouteGeometry.haversine_meters(start, end) * geometry_scale
                )
                if edge_meters == 0:
                    continue
                while route_meters + edge_meters + 1e-6 >= next_target_meters:
                    fraction = max(
                        0.0,
                        min(1.0, (next_target_meters - route_meters) / edge_meters),
                    )
                    ...
                    sections.append(
                        RunnerRouteSection(
                            f"Segment {next_mile:03d}{suffix} - {start_label} to {checkpoint_label} {next_mile:03d}",
                            tuple(section_coordinates),
                        ),
                    )
                    section_coordinates, start_label, suffix = (
                        [marker],
                        f"{checkpoint_label} {next_mile:03d}",
                        "",
                    )
                    next_mile += 1
                    next_target_meters += interval_meters
            ...
            if segment_index < len(segments) - 1:
                ...
                sections.append(
                    RunnerRouteSection(
                        f"Final segment - {start_label} to Finish",
                        tuple(section_coordinates),
                    ),
                )
        return sections
        ...
        for run_index, segment in enumerate(segments):
            ...
            for start, end in pairwise(segment):
                ...
                while run_meters + edge_meters + 1e-6 >= next_target_meters:
                    fraction = max(
                        0.0,
                        min(1.0, (next_target_meters - run_meters) / edge_meters),
                    )
                    marker = RouteGeometry.interpolate(start, end, fraction)
                    if section_coordinates[-1] != marker:
                        section_coordinates.append(marker)
            ...
            if len(section_coordinates) > 1:
                terminal = (
                    "support-car pickup" if run_index < len(segments) - 1 else "Finish"
                )
                sections.append(
                    RunnerRouteSection(
