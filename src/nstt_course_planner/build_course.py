...
from nstt_course_planner.config import (
    DEFAULT_GEOCODING_CACHE,
    DEFAULT_GOOGLE_USAGE,
    DEFAULT_OUTPUT_DIR,
    DEFAULT_PATH_CACHE,
...
from nstt_course_planner.models.route import (
    Point,
    RaceRouteSpec,
    RunnerRouteSection,
)
...
CHECKPOINTS = tuple(
    Checkpoint(point.label, point.query) for point in ROUTE_SPEC.route_checkpoints
)
...
class CourseBuilder:
    ...
    def __init__(
        ...
        route_spec: RaceRouteSpec = ROUTE_SPEC,
    ) -> None:
        self.config = config
        ...
        self.router = RunnerRouter(
            self.routing_cache,
            config.routing_cache_path,
            Environment.google_maps_api_key(),
            self.google_usage,
            config.google_usage_path,
        )
    ...
    def add_arguments(parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            ...
            help="Directory for KML, GPX, and import notes.",
        )
        parser.add_argument(
            "--geocoding-cache",
            type=Path,
            default=DEFAULT_GEOCODING_CACHE,
            help="Locally persisted public-geocoder results.",
        )
        parser.add_argument(
            "--routing-cache",
            type=Path,
            default=DEFAULT_ROUTING_CACHE,
            help="Locally persisted route geometry.",
        )
        parser.add_argument(
            "--path-cache",
            type=Path,
            default=DEFAULT_PATH_CACHE,
            help="Locally persisted named-path geometry.",
        )
        parser.add_argument(
            "--google-usage",
            type=Path,
            default=DEFAULT_GOOGLE_USAGE,
            help="Conservative local Google Routes request counter.",
        )
        parser.add_argument(
            "--approved-segments-kml",
            type=Path,
            help="Google My Maps KML containing user-edited route segments to preserve.",
        )
        parser.add_argument(
            "--approved-through-segment",
            type=int,
            help="Last consecutively approved source segment number.",
        )
        parser.add_argument(
            "--resume-at-checkpoint",
            default="Del Prado / Golden Lantern",
            help="Organizer checkpoint at which rerouting resumes.",
        )
        parser.add_argument(
            "--source-of-truth-kml",
            type=Path,
            help="KML whose route segments are retained exactly on both sides of a transfer gap.",
        )
        parser.add_argument(
            "--source-prefix-through-segment",
            type=int,
            help="Last immutable source segment before the transfer gap.",
        )
        parser.add_argument(
            "--source-prefix-end-marker",
            default="San Mateo Point",
            help="Named point that extends the final immutable source segment before the transfer gap.",
        )
        parser.add_argument(
            "--normalize-source-kml",
            type=Path,
            help="Manual runner KML whose exact geometry is re-cut into half-mile lines.",
        )
        parser.add_argument(
            "--official-directions-kml",
            type=Path,
            help="Verified organizer-directions KML copied into outputs as a reference layer.",
        )
    ...
    def from_arguments(cls, args: argparse.Namespace) -> CourseBuilder:
        return cls(
            CourseBuildConfig(
                args.output_dir,
                args.geocoding_cache,
                args.routing_cache,
                args.path_cache,
                args.google_usage,
                args.approved_segments_kml,
                args.approved_through_segment,
                args.resume_at_checkpoint,
                args.source_of_truth_kml,
                args.source_prefix_through_segment,
                args.source_prefix_end_marker,
                args.normalize_source_kml,
                args.official_directions_kml,
            ),
        )
    ...
    def resolve_points(self) -> list[Point]:
        ...
        for checkpoint in self.route_spec.route_checkpoints:
            ...
            point, from_cache = self.geocoder.geocode(
                Checkpoint(checkpoint.label, checkpoint.query),
                self.geocoding_cache,
            )
            points.append(point)
            if not from_cache:
                JsonStore.save_cache(
                    self.config.geocoding_cache_path,
                    self.geocoding_cache,
                )
                time.sleep(1.1)
        return points

    def build_from_approved_progress(
        self,
        points: list[Point],
    ) -> tuple[list[list[tuple[float, float]]], list[RunnerRouteSection]]:
        if (
            not self.config.approved_segments_kml
            or self.config.approved_through_segment is None
        ):
            raise RuntimeError(
                "Approved KML and approved-through segment are required for a progress rebuild.",
            )
        progress = ApprovedProgressLoader.load(
            self.config.approved_segments_kml,
            self.config.approved_through_segment,
        )
        try:
            resume_index = next(
                index
                for index, point in enumerate(points)
                if point.label == self.config.resume_at_checkpoint
            )
        except StopIteration as error:
            raise RuntimeError(
                f"Unknown resume checkpoint: {self.config.resume_at_checkpoint!r}.",
            ) from error
        if resume_index == 0:
        ...
        print(
            f"Preserving approved My Maps segments through {progress.sections[-1].label}; rerouting from its endpoint to {self.config.resume_at_checkpoint}.",
        )
        downstream, _ = self.router.trace(
    ...
    def build_from_source_of_truth(
        self,
    ) -> tuple[list[list[tuple[float, float]]], list[RunnerRouteSection]]:
        ...
        if source_path is None or through_segment is None:
            raise RuntimeError(
                "Source KML and source-prefix-through-segment are required for a source-of-truth rebuild.",
            )

        prefix = list(
            ApprovedProgressLoader.load(source_path, through_segment).sections,
        )
        tail = list(ApprovedProgressLoader.load_tail(source_path, through_segment + 1))
        marker_latitude, marker_longitude = ApprovedProgressLoader.named_point(
            source_path,
        ...
        print(
            f"Preserving source Segment 001 through Segment {through_segment:03d} exactly; adding only the walkable connector to {san_mateo.label}.",
        )
        ...
        if len(connector_segments) != 1 or len(connector_segments[0]) < 2:
            raise RuntimeError(
                "Could not create the runner connector from the source endpoint to San Mateo Point.",
            )
    ...
    def build_from_normalized_source(
        self,
    ) -> tuple[list[list[tuple[float, float]]], list[RunnerRouteSection]]:
        ...
        if source_path is None:
            raise RuntimeError(
                "A normalized source KML is required for a normalization rebuild.",
            )
        runs = ApprovedProgressLoader.source_geometry_runs(source_path)
        if len(runs) != 2:
            raise RuntimeError(
                f"Expected one runner run before and one after the support-car transfer, found {len(runs)} source runs.",
            )
        sections = RouteSegmenter.normalized_sections(
            runs,
            interval_meters=SEGMENT_METERS,
        )
    ...
    def build(self) -> None:
        if self.config.normalized_source_kml:
            ...
            if self.config.official_directions_kml:
                official_output = (
                    self.config.output_dir / "NSTT_2026_official_directions.kml"
                )
                shutil.copyfile(self.config.official_directions_kml, official_output)
                notes_path = (
                    self.config.output_dir / "NSTT_2026_runner_route_README.txt"
                )
            ...
            if self.config.approved_segments_kml:
                ...
                distance = RouteGeometry.distance_meters(segments)
                sections = RouteSegmenter.sections(
                    segments,
                    distance,
                    interval_meters=SEGMENT_METERS,
                    checkpoint_label="Checkpoint",
        ...
        print(
            f"Created {sum(len(segment) for segment in segments)} runner trace points in {len(segments)} runner runs, {distance / 1609.344:.1f} mi, {len(sections)} editable route lines.",
        )
