...
class RunnerRouteKml:
    ...
    def line_runs(
        ...
        if not runs:
            raise ValueError(
                f"Runner route source contains no usable LineString geometry: {source_path}",
            )
        return runs
...
class BathroomLayerBuilder:
    ...
    def build(
        ...
        source_path: Path,
        output_path: Path,
        stops: tuple[BathroomStop, ...] = BATHROOM_STOPS,
    ) -> BathroomLayerBuildResult:
        route_runs = RunnerRouteKml.line_runs(source_path)
        placemarks = "\n".join(
            self._placemark(stop, route_runs)
            for stop in sorted(
                stops,
                key=lambda stop: (stop.category.priority, stop.name),
            )
        )
    ...
    def _placemark(
        ...
        proximity = self.nearest_route_proximity(
        ...
        for run in route_runs:
            for start, end in pairwise(run):
                ...
                if distance < nearest:
                    nearest = distance
                    nearest_miles = (
                        route_meters + cls._segment_meters(start, end) * fraction
                    ) / 1609.344
                route_meters += cls._segment_meters(start, end)
        if math.isinf(nearest):
            raise ValueError(
                "Runner route geometry has no usable edges for bathroom distance calculations.",
            )
    ...
    def _point_to_segment(
        ...
        point_xy = (
            point[1] * longitude_scale,
            point[0] * cls.meters_per_degree_latitude,
        )
        start_xy = (
            start[1] * longitude_scale,
            start[0] * cls.meters_per_degree_latitude,
        )
        ...
        fraction = max(
            ...
            min(
                1.0,
                (
                    (point_xy[0] - start_xy[0]) * delta_x
                    + (point_xy[1] - start_xy[1]) * delta_y
                )
                / length_squared,
            ),
        )
        return math.dist(
            point_xy,
            (start_xy[0] + fraction * delta_x, start_xy[1] + fraction * delta_y),
        ), fraction
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a bathroom-stop KML beside the immutable runner route.",
    )
    parser.add_argument(
        "--runner-kml",
        type=Path,
        default=Path("input/Runner.kml"),
        help="Immutable runner route KML used only to calculate proximity.",
    )
    parser.add_argument(
        "--output-kml",
        type=Path,
        default=Path("outputs/NSTT_2026_bathroom_stops.kml"),
        help="Bathroom-layer KML to create.",
    )
